...
const LocalizeFiles = (props: LocalizeFilesProps) => {
    ...
    if (isConnected) {
      ...
      try {
        ...
        if(response?.data?.code == RESPONSE_CODE_SUCCESS){
          setCode('');
          setStorageAddress('');
          toast('success', 'top', 'SUCCESS!', t('fragment_localize_file_success'));
        } else if(response?.data?.code == RESPONSE_CODE_ERROR_FILE_BOX_CODE_NOT_FOUND){
          toast('error', 'top', 'ERROR!', t('fragment_localize_file_error_2'));
      ...
      const existingData = await AsyncStorage.getItem('offlineActions_localizeFile_'+userInfo.uniqueKey);
      ...
      await setOfflineDataIntoStorage('offlineActions_localizeFile_'+userInfo.uniqueKey, JSON.stringify(offlineActions));
      toast('info', 'top', 'SUCCESS!', t('fragment_localize_file_success'));
    }
  };
    ...
    <View style={styles.tabContainer}>
      ...
      <View style={Styles.mb_10}>
            <InputOutlined
              ...
              rightIcon={<TextInput.Icon icon="qrcode-scan" onPress={() => props.openScanner('lf')} />}
            />
        </View>
        <View style={Styles.mb_10}>
            <InputOutlined
              ...
              rightIcon={<TextInput.Icon icon="qrcode-scan" onPress={() => props.openScanner('lf_sa')} />}
            />
        </View>
        <View>
          <Button variant="general" title={t('fragment_localize_file_title')} onClickHandler={() => sendAction()} buttonCssClass={[{width: '100%'}]} />
        </View>
      </ScrollView>
